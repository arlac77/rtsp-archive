...
expand(program.config ? "${include('" + path.basename(program.config) + "')}"
 : {}).then(config => {
   ...
   let recorders = config.recorders || {};
   ...
   browser.on('serviceUp', service => {
    ...
    for(let i in service.addresses) {
       ...
       if(a.match(/^[0-9\.]+$/)) { recorder.address = a; break; }
    }
 ...
 function startRecording(config,recorderName) {
 	...
 	const dir = path.join(config.record.dir, recorderName, Date.format(today, 'Y'), Date.format(today, 'm'), Date.format(today, 'd'));
